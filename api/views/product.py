...
from api.utilities.constants.constants import USR_02, USR_05
...
@api.route('/categories/inProduct/<int:product_id>')
...
class ProductsIndepartment(Resource):
    ...
    def get(self, department_id):
        ...
        truncate_description(data, description_length)
        response = jsonify(
            {'count': len(data), 'status': 'success', 'rows': data}
        )
        return response


@api.route('/products/<int:product_id>/details')
...
@api.route('/products/<int:product_id>/locations')
    ...
    def get(self, product_id):
        ...
        if not reviews:
            raise ValidationError(
                {'message': serialization_errors['not_found'].format('product with this id')}, USR_05, 'produc_id')
